...
class AddHeatReading (
  ...
  // Используем GetSimpleResponse? для поддержки nullable в Resource.Success
  operator fun invoke(addReadingParams: AddHeatReadingParams): Flow<Resource<GetSimpleResponse?>> =
    flow<Resource<GetSimpleResponse?>> {
      ...
      try {
        ...
        if (response.success == 1) {
          ...
          // Выводим сообщение об ошибке, пришедшее от PHP
    ...
    }.flowOn(Dispatchers.IO) // Гарантирует выполнение сети вне Main Thread
}
